...
CREATE TABLE recipe_tags (
  ...
  FOREIGN KEY (tag_id) REFERENCES tags (id) ON DELETE CASCADE
    ...
    // Step photos table — FR-033: per-step photo attachments
    await db.execute('''
...
CREATE TABLE IF NOT EXISTS step_photos (
  ...
  // Step photo operations (FR-033)

  /// Upsert a photo path for a step. Pass null to remove.
  Future<void> setStepPhoto(String recipeId, int stepIndex, String photoPath) async {
    final db = await database;
    await db.insert(
      'step_photos',
      {'recipe_id': recipeId, 'step_index': stepIndex, 'photo_path': photoPath},
  ...
  Future<void> deleteStepPhoto(String recipeId, int stepIndex) async {
    ...
    await db.delete(
      ...
      whereArgs: [recipeId, stepIndex],
    );
  }

  /// Returns a map of stepIndex -> photoPath for all steps of a recipe.
  Future<Map<int, String>> getStepPhotos(String recipeId) async {
    final db = await database;
    ...
    return {
      for (final row in rows)
        (row['step_index'] as int): row['photo_path'] as String,
    };
  }

  /// Delete all step photos for a recipe (used when recipe is deleted).
  Future<void> deleteAllStepPhotos(String recipeId) async {
    final db = await database;
    await db.delete(
      'step_photos',
      where: 'recipe_id = ?',
